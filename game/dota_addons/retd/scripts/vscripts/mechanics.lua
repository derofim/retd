function updateStatGUI( player, radiantLifes, direLifes )
	CustomGameEventManager:Send_ServerToPlayer(player, "stat_gui_changed", { 
		radiant_lifes = math.floor(radiantLifes), dire_lifes = math.floor(direLifes) })
end

function ModifyFood( player, value )
	...
	if value > 0 then
		...
		if PlayerHasEnoughLumber( player, math.abs(value) ) then
			player.food = player.food + value
		    CustomGameEventManager:Send_ServerToPlayer(player, "player_food_changed", { food = math.floor(player.food) })
		end
	end
...
function ModifyLifes( player, value )
	-- if value == 0 then return end
	if value > 0 then
		player.lifes = player.lifes + value
	    CustomGameEventManager:Send_ServerToPlayer(player, "player_lifes_changed", { lifes = math.floor(player.lifes) })
	else
		if PlayerHasEnoughLifes( player, math.abs(value) ) then
			player.lifes = player.lifes + value
		    CustomGameEventManager:Send_ServerToPlayer(player, "player_lifes_changed", { lifes = math.floor(player.lifes) })
		end
	end
...
function GetLifesCost( unit )
	if unit and IsValidEntity(unit) then
		if unit.LifesCost then
			return unit.LifesCost
		end
	end
	return 0
end

function PlayerHasEnoughLifes( player, lifes_cost )
	local pID = player:GetAssignedHero():GetPlayerID()
	if(lifes_cost == nil) then 
		return true
	end

	if player.lifes < lifes_cost then
		SendErrorMessage(pID, "#error_not_enough_lifes")
		return false
	else
		return true
	end
