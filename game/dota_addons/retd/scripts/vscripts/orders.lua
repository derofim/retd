function CustomGameMode:FilterExecuteOrder( filterTable )
    ...
    --           Ability Multi Order              --
    ------------------------------------------------
    if (    order_type ~= DOTA_UNIT_ORDER_PURCHASE_ITEM 
        and order_type ~= DOTA_UNIT_ORDER_GIVE_ITEM 
        and order_type ~= DOTA_UNIT_ORDER_PICKUP_ITEM 
        and order_type ~= DOTA_UNIT_ORDER_SELL_ITEM 
        and order_type ~= DOTA_UNIT_ORDER_MOVE_ITEM 
        and abilityIndex 
        and abilityIndex ~= 0 
        and EntIndexToHScript(abilityIndex)~=nil 
        and IsMultiOrderAbility(EntIndexToHScript(abilityIndex)) ) then
    ...
    elseif (order_type ~= DOTA_UNIT_ORDER_PURCHASE_ITEM 
        and order_type ~= DOTA_UNIT_ORDER_GIVE_ITEM 
        and order_type ~= DOTA_UNIT_ORDER_PICKUP_ITEM 
        and order_type ~= DOTA_UNIT_ORDER_SELL_ITEM 
        and order_type ~= DOTA_UNIT_ORDER_MOVE_ITEM 
        and abilityIndex and abilityIndex ~= 0) and IsBuilder(unit) then
        local ability = EntIndexToHScript(abilityIndex)
