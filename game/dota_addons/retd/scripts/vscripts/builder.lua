...
function Build( event )
	...
	local lifes_cost = ability:GetSpecialValueFor("lifes_cost")
	...
	if not PlayerHasEnoughLifes( player, lifes_cost ) then
		return
	end
	...
	event:OnPreConstruction(function(vPos)
		...
		if not PlayerHasEnoughGold( player, gold_cost ) then
       	...
       	if not PlayerHasEnoughFood( player, food_cost ) then
       		SendErrorMessage(caster:GetPlayerOwnerID(), "#error_not_enough_food")
			return false
		end

       	if not PlayerHasEnoughLifes( player, lifes_cost ) then
       		SendErrorMessage(caster:GetPlayerOwnerID(), "#error_not_enough_lifes")
			return false
		end
	...
	event:OnConstructionStarted(function(unit)
    	...
    	if (GetMapName() == "2players" or GetMapName() == "4players" or GetMapName() == "2builder") then
    		if(not GridNav:CanFindPath(Entities:FindByName( nil, "radiant_base_spawn"):GetAbsOrigin(), Entities:FindByName( nil, "dire_base_spawn"):GetAbsOrigin())) then
    			local keys = {}
    			keys.unit = unit
    			BuildingHelper:CancelBuilding(keys)
    			SendErrorMessage(caster:GetPlayerOwnerID(), "#error_invalid_build_position")
    		end
    	elseif (GetMapName() == "retd") then
    		if(        not GridNav:CanFindPath(Entities:FindByName( nil, "radiant_base_spawn"):GetAbsOrigin(), Entities:FindByName( nil, "dire_base_spawn"):GetAbsOrigin())
    				or not GridNav:CanFindPath(Entities:FindByName( nil, "dire_easy_1"):GetAbsOrigin(), Entities:FindByName( nil, "dire_base"):GetAbsOrigin())
    				or not GridNav:CanFindPath(Entities:FindByName( nil, "rad_easy_1"):GetAbsOrigin(), Entities:FindByName( nil, "radiant_base"):GetAbsOrigin())
    				or not GridNav:CanFindPath(Entities:FindByName( nil, "hard_dire1"):GetAbsOrigin(), Entities:FindByName( nil, "hard_dire13"):GetAbsOrigin())
    				or not GridNav:CanFindPath(Entities:FindByName( nil, "hard_rad1"):GetAbsOrigin(), Entities:FindByName( nil, "hard_rad15"):GetAbsOrigin())
    			) then
    			local keys = {}
    			keys.unit = unit
    			BuildingHelper:CancelBuilding(keys)
    	...
    	elseif (GetMapName() == "redota") then
    		if(        not GridNav:CanFindPath(Entities:FindByName( nil, "lane_top_pathcorner_goodguys_1"):GetAbsOrigin(), Entities:FindByName( nil, "lane_top_pathcorner_badguys_1"):GetAbsOrigin())
    				or not GridNav:CanFindPath(Entities:FindByName( nil, "lane_mid_pathcorner_goodguys_1"):GetAbsOrigin(), Entities:FindByName( nil, "lane_mid_pathcorner_badguys_1"):GetAbsOrigin())
    				or not GridNav:CanFindPath(Entities:FindByName( nil, "lane_bot_pathcorner_goodguys_1"):GetAbsOrigin(), Entities:FindByName( nil, "lane_bot_pathcorner_badguys_1"):GetAbsOrigin())
    			) then
    			local keys = {}
    			keys.unit = unit
    			BuildingHelper:CancelBuilding(keys)
    		...
    		DebugPrint("[BUILDER.LUA] !!!!!!!!!!!!!!  NEED FIX CanFindPath  !!!!!!!!!!!!!!!  ")
    	end
	end)
...
function Repair( event )
	...
	local food_cost = building.FoodCost
