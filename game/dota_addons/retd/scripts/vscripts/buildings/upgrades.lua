...
function UpgradeBuilding( event )
	...
	local casterInventory = {}
	for i=0,5 do
        local item = caster:GetItemInSlot(i)
        if item then
        	if item:GetAbilityName() ~= "item_building_cancel" then
        		table.insert(casterInventory, item:GetAbilityName())
        	end
        end
    end
	...
	local old_building_name = caster:GetUnitName()
	local old_gold_cost = caster.GoldCost
	local old_lumber_cost = caster.LumberCost
	local old_food_cost = caster.FoodCost
	local old_build_time = caster.BuildTime
	...
	for i=1,#casterInventory do
		building:AddItemByName(casterInventory[i])
	end
	...
	building.GoldCost = old_gold_cost + gold_cost
	building.LumberCost = old_lumber_cost + lumber_cost
	building.FoodCost = old_food_cost + food_cost
	building.BuildTime = old_build_time + build_time

	--building:AddNewModifier(unit, nil, "modifier_invulnerable", nil)
...
function StartUpgrade( event )	
	...
	if (caster.queue~=nil and #caster.queue < 5 and #caster.queue > 0) then -- fix
...
function CancelUpgrade( event )
	...
	if(abilities==nil) then abilities = {} end
