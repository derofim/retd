...
function EnqueueUnit( event )
    ...
    hero:ModifyGold(gold_cost, true, 0) -- give gold if cant upgrade
    ...
    hero:ModifyGold(-gold_cost, true, 0) -- return gold

    ModifyLumber( hero:GetPlayerOwner(), -lumber_cost)
    ModifyFood( hero:GetPlayerOwner(), -food_cost)
...
function DequeueUnit( event )
	...
	local ability = train_ability -- fixed
	local build_time = ability:GetSpecialValueFor("build_time")
	local gold_cost = ability:GetSpecialValueFor("gold_cost")
	local lumber_cost = ability:GetSpecialValueFor("lumber_cost")
	local food_cost = ability:GetSpecialValueFor("food_cost")
